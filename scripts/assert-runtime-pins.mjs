...
import { compareVersions, extractNodeBounds } from "./node-version-range.mjs";
...
if (typeof range === "string") {
  ...
  if (minimum !== undefined && compareVersions(current, minimum) < 0) {
    // Below the minimum the code cannot be expected to run at all.
    fail(
      `Node ${current} is below the required ${minimum} (engines: ${range})`,
    );
  }
  if (maximum !== undefined && compareVersions(current, maximum) >= 0) {
    // Canonical verification is runtime evidence, not a convenience lint. A
    // run outside the declared upper bound must fail; otherwise Node 24 can
    // produce a green artifact for a Node 22 project.
    fail(
      `Node ${current} is outside the supported range ${range}; canonical verification requires the pinned runtime`,
    );
  }
  ...
  for (const pinFile of [".node-version", ".nvmrc"]) {
    ...
    if (compareVersions(pinned, minimum ?? pinned) < 0) {
      fail(
        `${pinFile} (${pinned}) is below engines.node's minimum (${minimum})`,
      );
    }
    if (maximum !== undefined && compareVersions(pinned, maximum) >= 0) {
