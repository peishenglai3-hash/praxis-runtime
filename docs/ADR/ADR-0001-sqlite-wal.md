...
Use SQLite with WAL, foreign keys enabled, `synchronous=FULL`, and a configured busy timeout. All event writes go through one EventWriter. Use the Node.js built-in `node:sqlite` `DatabaseSync` API with a minimum Node.js version of `22.13.0`, the first 22.x line where the module is unflagged.
