...
**Status:** `PARTIAL` — EPIC-008 is complete; EPIC-009's issue 081 is
`PARTIAL`. An earlier version of this header read `PASS`; the P6-R0 audit on
2026-09-16 re-derived the status from the tree and found the 081 row the only
outstanding acceptance condition. Everything else this gate claims is backed by
executable coverage.
...
| 085   | `legacy` import `--dry-run` / commit, with a report path.                                        | `praxis legacy import --dry-run` / `--confirm --plan-hash`; `praxis legacy runs` / `anomalies`. **Closed 2026-09-16:** `--report <path>` writes the migration report on both the dry run and the commit. The document is built by the runtime (`packages/runtime/src/legacy/report.ts`) at the same construction site the ledger uses, so the two cannot describe a corpus differently; the CLI only decides where the bytes land. Covered by `tests/unit/legacy-report.test.ts` (6) and three cases in `tests/integration/phase5-cli.test.ts`. | `PASS` locally |
